...
impl<'m> CallGraph<'m> {
    ...
    pub fn print_call_graph(&self) -> Result<(), Box<dyn Error>> {
        let mut file = File::create("cg.txt")?;

        writeln!(file, "=== Call Graph ===")?;
        writeln!(file)?;

        for func in self.graph.nodes() {
            writeln!(file, "Function: {}", func)?;

            writeln!(file, "  Callees:")?;
            for callee in self.graph.neighbors_directed(func, Direction::Outgoing) {
                writeln!(file, "    {}", callee)?;
            }

            writeln!(file, "  Callers:")?;
            for caller in self.graph.neighbors_directed(func, Direction::Incoming) {
                writeln!(file, "    {}", caller)?;
            }
